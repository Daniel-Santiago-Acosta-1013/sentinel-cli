...
fn render_detail(
    ...
    let body = match session.route {
        Route::Safety => output::render_safety_table(
...
fn render_recovery(
    ...
    let compact = terminal_width < 84;
    let table = output::render_recovery_table(
        &session.runtime_state,
        terminal_width,
        compact,
        profile,
...
fn render_progress(
    ...
    let mut lines = vec![
        ...
        String::new(),
        styles::emphasis(
            &spinner::line(progress, session.progress_step, profile),
            profile,
        ),
        String::new(),
        styles::muted("La vista final aparecera al terminar la operacion.", profile),
    ];
    if width > 60 {
        lines.push(String::new());
...
fn render_result(
    ...
    let result = session.last_result.as_ref().expect("result route requires last_result");
    let compact = terminal_width < 84;
...
fn render_menu(
    ...
    for (index, action) in actions.iter().enumerate() {
        ...
        if selected && !action.description.is_empty() && action.id != MenuActionId::Exit {
