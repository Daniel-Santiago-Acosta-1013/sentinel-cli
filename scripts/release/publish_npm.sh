...
package_version=$(release_package_json_version)
existing_status=$(release_read_state_value "npm" STATUS)
existing_version=$(release_read_state_value "npm" VERSION)

[ "$package_version" = "$RELEASE_VERSION" ] || \
  release_fail "npm package template version does not match release manifest"
...
cp "$template_dir/package.json" "$stage_dir/package.json"
